...
class CSVGGraph
{
    ...
    function drawElement( $x, $y, $w, $string, $type )
    {
        ...
        if ( $this->color )
        {
            ...
            if ( ETYPE_LEAF_LINK == $type )
                $col = "darkorange";

            if ( ETYPE_LEAF_MATCH == $type || ETYPE_LEAF_LINK_MATCH == $type )
                $col = "red";
        ...
        if ( ETYPE_LEAF_LINK == $type || ETYPE_LEAF_LINK_MATCH == $type ) {
        ...
        $this->xml .= sprintf( "<text %s x=\"%d\" y=\"%d\" text-decoration=\"%s\">%s</text>\n",
            $style, $txt_pos, $top+($this->e_height-5), $text_decoration, htmlentities( $main ) );
        
        // Draw subscript text

        if ( strlen( $sub ) > 0 )
        {
            $style=sprintf( "style=\"fill: %s; font-size: %dpx;\"", $col, $this->font_size -3 );
